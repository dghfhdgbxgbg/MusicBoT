...
from AMBOTMusicBoT.Helpers.mongo import mongodb
...
async def add_banned_user(user_id: int, reason: str, current_time: str, gban_by: str):
    is_gbanned = await is_banned_user(user_id)
    if is_gbanned:
        return
    return await blockeddb.insert_one({"user_id": user_id, "reason": reason, "current_time": current_time, "gban_by": gban_by})
...
async def get_info_ban(user_id: int) -> dict:
    banned_user = await blockeddb.find_one({"user_id": user_id})
    if banned_user:
        return {
            "user_id": banned_user.get("user_id"),
            "reason": banned_user.get("reason"),
            "current_time": banned_user.get("current_time"),
            "gban_by": banned_user.get("gban_by")
        }
    else:
        return {}


async def add_approved_chats(user_id: int, chat_id: int):
...
async def remove_approved_chats(user_id: int, chat_id: int):
    await blockeddb.update_one(
        {"user_id": user_id},
        {"$pull": {"chats": chat_id}}
    )

async def get_approved_chats(user_id: int) -> list:
    approved_chats = await blockeddb.find_one({"user_id": user_id})
    return approved_chats.get("chats", []) if approved_chats else []
